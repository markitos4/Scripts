if (not getgenv) or (not generateramdon) or (not gethui) or (not newcclosure) or (not getgenv().identifyexecutor() == "Arctic") then return error("Missing an important function (Drawing Lib)") end 
...
return setmetatable({}, {
	__newindex = function(_, index, value)
		if lineObj[index] == nil then 
			error("Invalid property provided: " .. tostring(index))
			return 
		end
...
return setmetatable({Parent = drawingUI}, {
	__newindex = function(_, index, value)
		if textObj[index] == nil then 
			return error("Invalid property provided: " .. tostring(index)) 
		end
...
function DrawingLib.createCircle()
	...
	circleFrame.Name = generateramdon()
...
squareFrame.Name = generateramdon()
...
imageFrame.Name = generateramdon()
...
getgenv().Drawing.Fonts = table.freeze({
  ["UI"] = 0,
  ["System"] = 1,
  ["Plex"] = 2,
  ["Monospace"] = 3
...
getgenv().Drawing.new = newcclosure(function(drawingType)
